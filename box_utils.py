...
def boxes3d_kitti_camera_to_lidar(boxes3d_camera):
    """
    Args:
...
def boxes3d_to_corners3d_kitti_camera(boxes3d, bottom_center=True):
 ...
 ## 每个盒子是八行三列，从中找出xy的边界
def boxes3d_kitti_camera_to_imageboxes(boxes3d, image_shape=None):
    """
    ...
    print(corners3d.shape)   ##(15, 8, 3)  15
    ## 每一个8*3的 找出xy最大值最小值

    ##一帧中一共有num_box个盒子
    num_box = len(corners3d)
    ## 创建一个空array，里面将要存放所有box的2d边界
    boxes2d_image = np.zeros((num_box,4))

    ## for循环，遍历所有box，逐个求2d信息
    for i in range(1,num_box+1):
        ...
        ## 将每个box的2d信息存入array中
        boxes2d_image[i-1,0] = xmin
        boxes2d_image[i-1,1] = ymin
        boxes2d_image[i-1,2] = xmax
        boxes2d_image[i-1,3] = ymax
