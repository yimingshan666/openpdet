...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        # print(num_valid_gt)
    # for i in range(num_gt):
        # 2.4如果类别是DontCare，则在dc_bboxes中加入该box,自己的数据里无
...
## 根据计算出的overlaps=0，定位到了这里！！
## 循环输入时，有很多dontcare的，因此有很多0，正常
...
def image_box_overlap(boxes, query_boxes, criterion=-1):
    ...
    Args:
        boxes:一个part中的全部det
        query_boxes：一个part中的全部gt
    """

    N = boxes.shape[0] # det_box的总数
    K = query_boxes.shape[0] # gt_box的总数
    # print(N)
    # print(K)
...
## 计算出的iou值前两个全为0，有问题，定位到了这里！！！（输入的labels等没问题）
...
## 从eval_class输出全为0定位到这里,然后发现clean_data有问题，输出的好多0
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    ## 发现total_dc_num, total_num_valid_gt全为0,前者=0正常，后者=0，不正常
    return (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets, dontcares,
            total_dc_num, total_num_valid_gt)

## 从计算ap值计算出错开始，问题追踪到这里了！！
## 这里有问题，输出的全是0
def eval_class(gt_annos,
    ...
    # 2、计算iou 计算了三次iou，分别为2d dev 3d
    rets = calculate_iou_partly(dt_annos, gt_annos, metric, num_parts)
    overlaps, parted_overlaps, total_dt_num, total_gt_num = rets

    # overlaps每个元素代表了一帧点云的iou
    # parted_overlaps表示每个part的iou
    # total_gt_num 表示每帧点云的gt_box的数量
    # total_dt_num 表示每帧点云的det_box的数量
    ##iou计算结果，看起来正常的
    # print(rets)
    ## 额外的
    # print('overlaps')
    ...
    for m, current_class in enumerate(current_classes):
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                # 4.2.1 计算所有有效gt匹配上的dt的全部分数，所有box score和recall阈值点
                for i in range(len(gt_annos)):
                    # 这里调用compute_statistics_jit没有真正计算指标，而是获取thresholdss，然后计算recall阈值点
                ...
                # 4.2.2 遍历所有part，在part内逐帧逐recall_threshold计算tp, fp, fn, similarity，累计pr
                for j, num_part in enumerate(split_parts):
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    Args:
        gt_annos:帧的测试点云标注字典，包括{name,alpah,rotation_y,gt_boxes_lidar}等信息
        dt_annos:帧的测试点云预测字典，包括{name,alpah,rotation_y,boxes_lidar}等信息
    """
    ...
    ## min_overlaps = 把overlap_0_7和overlap_0_5 上下叠在一起
